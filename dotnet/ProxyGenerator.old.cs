...
namespace EdgeReference
{
	public class ProxyGenerator
	{
		...
		private void Generate(Type target)
		{
			...
			this.GenerateRequires (target);
			this.GenerateClassDefinition (target);
			...
			staticProperties.ForEach ((info) => { 
				this.GenerateProperty(info, true);
			});
			...
			instanceMethods.ForEach ((info) => {
				this.GenerateFunction (info, false);
			});
